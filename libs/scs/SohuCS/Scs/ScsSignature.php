...
/**
 * Default Amazon Scs signature implementation
 * @link http://docs.aws.amazon.com/AmazonScs/latest/dev/RESTAuthentication.html
 */
class ScsSignature implements ScsSignatureInterface
{
    /**
    ...
    public function signRequest(RequestInterface $request, CredentialsInterface $credentials)
    {
        ...
        if ($token = $credentials->getSecurityToken()) {
            $request->setHeader('x-scs-security-token', $token);
        }

        // Add a date header if one is not set
        if (!$request->hasHeader('date') && !$request->hasHeader('x-scs-date')) {
            $request->setHeader('Date', gmdate(\DateTime::RFC2822));
        }

        $stringToSign = $this->createCanonicalizedString($request);
        $request->getParams()->set('scs.string_to_sign', $stringToSign);
    ...
    public function createPresignedUrl(
        ...
        $path = ScsClient::encodeKey(rawurldecode($request->getPath()));
        $request->setPath($path);

        // Make sure to handle temporary credentials
        if ($token = $credentials->getSecurityToken()) {
            $request->setHeader('x-scs-security-token', $token);
            $request->getQuery()->set('x-scs-security-token', $token);
        }

        // Set query params required for pre-signed URLs
        $request->getQuery()
            ->set('SCSAccessKeyId', $credentials->getAccessKeyId())
            ->set('Expires', $expires)
    ...
    {
        $buffer = strtolower($request->getMethod()) . "\n";
        ...
        $buffer .= "{$date}\n"
            . $this->createCanonicalizedScsHeaders($request)
     ...
     */
    private function createCanonicalizedScsHeaders(RequestInterface $request)
    {
        $headers = array();
        foreach ($request->getHeaders() as $name => $header) {
            $name = strtolower($name);
    ...
    {
        $buffer = $request->getParams()->get('scs.resource');
        ...
        if (null === $buffer) {
            ...
            // Remove encoding from the path and use the Scs specific encoding
            $path = ScsClient::encodeKey(rawurldecode($request->getPath()));
    ...
    private function parseBucketName(RequestInterface $request)
    {
        ...
        if (strpos($host, $baseHost) === false) {
            ...
            $baseHost = '';
            // For every known Scs host, check if that host is present on the request
