$(function(){
	...
	/* 相关房源横向滚动 */
	function recSlider(obj,stepline,steplength,stepwidth){
		var $slider = $(obj),
		...
		var len = $li.length,
			currentPage = 0, 
			pageSize = steplength * stepline,
			totalPage = parseInt((len + pageSize -1) / pageSize);		
		var lastPageSize = len - pageSize * (totalPage - 1);
		lastPageSize = lastPageSize > steplength ? steplength : lastPageSize;
		var lastPageWidth = lastPageSize * stepwidth;
		w = stepwidth * steplength * (totalPage-1) + lastPageWidth;	
		$ul.width(w);
		if(totalPage == 1){
			$prev.hide();
			$next.hide();
		...
		function scrollRight(){	
			...
			$ul.stop(true,false).animate({left: -stepwidth * currentPage * steplength},300);
		}
	...
	recSlider("#price_rec",2,3,313);
	recSlider("#area_rec",1,5,190);
})
...
$(function(){
	...
	var loanType = $loanType.find(".dropdown_btn span").attr("data-value"),
		...
		loanProp = $refer.find("input[name='goufangxingzhi']:checked").val(),
		monthPay = 0,
		loanTotal = 0,
		firstPay = 0,
		interestPay = 0,
	...
	$refer.find("input[name='goufangxingzhi']").click(function(){
		if($(this).val() == 1){
			loanProp = 1;
		}else if($(this).val() == 2){
			loanProp = 2;
	...
	function monthLegend(loanType,loanLtv,loanYears){
		...
		firstPay = totalPrice * loanLtv * 0.1;
		$firstPay.find("span:first").text(Math.round(firstPay/10000));
		$firstPay.find("span:last").text(loanLtv);
		
		var $loanAmount = $("#loan_amount");
		loanTotal = totalPrice * (10-loanLtv) * 0.1;
		$loanAmount.find("span:first").text(Math.round(loanTotal/10000));
		$loanAmount.find("span:last").text(10-loanLtv);
	...
	function getRate(loanType,loanLtv,loanYears){
		...
		if(loanProp == 1){
			...
			loanRate = loanRate * 1.1;
		}
	}
	
	/* 购房性质提示 */
	$refer.find(".tips").hover(function(){
		$(this).find(".tips_box").toggle();
	});
});
