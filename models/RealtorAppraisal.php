 ...
 * @since  2014年10月8日19:34:21
 */
class RealtorAppraisal extends BaseModel
{
	...
	public function columnMap()
	{
		return array(
				'realId' => 'realId',
				'comId' => 'comId',
				'raSetTodayRelease' => 'publish',
				'raSetTodayRefresh' => 'fresh',
				'raSetTotalRelease' => 'published',
				'raSetTotalTag' => 'tag',
				'raSetTotalGreat' => 'fine',
				'raSetTotalPicture' => 'picture',
				'raSetTotalFailure' => 'failure',
				'raValTotayRelease' => 'publishRes',
				'raValTotayRefresh' => 'freshRes',
				'raValTotalRelease' => 'publishedRes',
				'raValTotalTag' => 'tagRes',		
				'raValTotalGreat' => 'fineRes',
				'raValTotalPicture' => 'pictureRes',
				'raValTotalFailure' => 'failureRes',
				'raLastTime' => 'examTime',
				'raDoneCount' => 'useStat',
				'raLastScore' => 'lastScore'
		);
	}
	...
	public static function getExamDefault($arrRealtorInfo)
	{
		...
		if($arrRealtorInfo['pcType'] == '10'){
			return array(
					...
					'fresh' => intval($arrRealtorInfo['opSaleRefresh']),
					'published' => intval($arrRealtorInfo['opSaleRelease']),
					'tag' => intval($arrRealtorInfo['opSaleTags']),
					'fine' => intval($arrRealtorInfo['opSaleBold']),
					'picture' => intval($arrRealtorInfo['opSaleRelease']),
	...
	public function checkPortExam(array $arrPost, $arrRealtorInfo){
		...
		foreach ($arrExamDef as $key => $val){
			if(intval($arrPost[$key]) < 0){//考核数量端口权限值可以为0，所以限制不能小于0
				$this->error = $arrExamName[$key].'考核数量不能为负数';
				return false;
			}
			if(($key == 'publish' && $val > 9999) || ($key != 'publish' && $arrPost[$key] > $val)){//今日发布限制输入4位，则最大值9999
				$this->error = $arrExamName[$key].'考核数量不能大于端口权限';
				return false;
			}
		}
		return true;
	...
	public function InsertExamSet(array $arrExam, $arrRealtorInfo){
		...
		$arrData = self::findFirst(" realId = ".$arrExam['realId']);
