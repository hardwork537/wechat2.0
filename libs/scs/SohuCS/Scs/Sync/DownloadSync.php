...
use SohuCS\Scs\ResumableDownload;

/**
 * Downloads and Amazon Scs bucket to a local directory
 */
class DownloadSync extends AbstractSync
{
    protected function createTransferAction(\SplFileInfo $file)
    {
        ...
        // Some Scs buckets contains nested files under the same name as a directory
        if (is_dir($filename)) {
            return false;
        }
    ...
    protected function createDirectory($filename)
    {
        ...
        // Some Scs clients create empty files to denote directories. Remove these so that we can create the directory.
