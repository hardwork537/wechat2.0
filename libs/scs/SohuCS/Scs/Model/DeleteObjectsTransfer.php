...
namespace SohuCS\Scs\Model;

use SohuCS\Common\Client\SohuCSClientInterface;
use SohuCS\Common\Exception\OverflowException;
use SohuCS\Common\Enum\UaString as Ua;
use SohuCS\Scs\Exception\InvalidArgumentException;
use SohuCS\Scs\Exception\DeleteMultipleObjectsException;
use Guzzle\Batch\BatchTransferInterface;
use Guzzle\Service\Command\CommandInterface;

/**
 * Transfer logic for deleting multiple objects from an Amazon Scs bucket in a
 * single request
 */
class DeleteObjectsTransfer implements BatchTransferInterface
{
    /**
     * @var SohuCSClientInterface The Amazon Scs client for doing transfers
     */
    protected $client;
     ...
     *
     * @param SohuCSClientInterface $client Client used to transfer the requests
     * @param string             $bucket Name of the bucket that stores the objects
     * @param string             $mfa    MFA token used when contacting the Amazon Scs API
     */
    public function __construct(SohuCSClientInterface $client, $bucket, $mfa = null)
    {
        $this->client = $client;
        $this->bucket = $bucket;
        $this->mfa = $mfa;
