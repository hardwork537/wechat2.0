...
//                )
//            );
//            $this->mCache->addServers($cluster);
            foreach ($cluster AS $one) {
                $this->mCache->addserver($one['host'], $one['port'],  $one['weight']);
            }
        }
        ...
        public static function Instance($cluster=array()) {
            ...
            if ( isset( self::$mInstance[ $cluster_key ] ) && self::$mInstance[ $cluster_key ]) return self::$mInstance[ $cluster_key ];
            return self::$mInstance[ $cluster_key ] = new MCache($cluster);//注意请不要把这里Mem改成MemCache，这里是用类名
        }
        ...
        public function Add($key, $var, $expire=0) {
            return $this->mCache->add(trim($key),$var,$expire);
        }
        ...
        public function Replace($key, $var, $expire=0) {
            return $this->mCache->replace(trim($key), $var, $expire);
        }
        ...
        public function Set($key, $var, $expire=0) {
            return $this->mCache->set(trim($key), $var,$expire);
        }
        ...
        public static function Flush($delay = 0) {
         //   self::Instance();
            return self::Instance()->mCache->flush($delay);
        }

        public function getResultMessage(){
            return $this->mCache->getResultMessage();
        }
    }