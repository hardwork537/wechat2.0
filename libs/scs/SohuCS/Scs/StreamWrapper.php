...
use SohuCS\Scs\Exception\NoSuchKeyException;
use SohuCS\Scs\Iterator\ListObjectsIterator;
...
/**
 * Amazon Scs stream wrapper to use "scs://<bucket>/<key>" files with PHP streams, supporting "r", "w", "a", "x".
 *
 ...
 * by setting the "seekable" stream context option true. This will allow true streaming of data from Amazon Scs, but
 * will maintain a buffer of previously read bytes in a 'php://temp' stream to allow seeking to previously read bytes
 * from the stream.
 *
 * You may pass any GetObject parameters as 'scs' stream context options. These options will affect how the data is
 * downloaded from Amazon Scs.
 *
 * # Opening "w" and "x" (write only) streams:
 *
 * Because Amazon Scs requires a Content-Length header, write only streams will maintain a 'php://temp' stream to buffer
 * data written to the stream until the stream is flushed (usually by closing the stream with fclose).
 *
 * You may pass any PutObject parameters as 'scs' stream context options. These options will affect how the data is
 * uploaded to Amazon Scs.
 *
 * When opening an "x" stream, the file must exist on Amazon Scs for the stream to open successfully.
 *
 ...
 * streams will attempt to download the contents of an object in Amazon Scs, seek to the end of the object, then allow
 ...
 *
 * You may pass any GetObject and/or PutObject parameters as 'scs' stream context options. These options will affect how
 * the data is downloaded and uploaded from Amazon Scs.
 *
...
class StreamWrapper
{
    ...
    /**
     * @var ScsClient Client used to send requests
     */
    ...
    /**
     * Register the 'scs://' stream wrapper
     *
     * @param ScsClient $client Client to use with the stream wrapper
     */
    public static function register(ScsClient $client)
    {
        if (in_array('scs', stream_get_wrappers())) {
            stream_wrapper_unregister('scs');
        }

        stream_wrapper_register('scs', get_called_class(), STREAM_IS_URL);
        self::$client = $client;
    }
    ...
    public function stream_open($path, $mode, $options, &$opened_path)
    {
        ...
        if (!$params['Key']) {
            $errors[] = 'Cannot open a bucket. You must specify a path in the form of scs://bucket/key';
        }

        if (strpos($mode, '+')) {
            $errors[] = 'The Amazon Scs stream wrapper does not allow simultaneous reading and writing.';
        }
        ...
        if ($mode == 'x' && self::$client->doesObjectExist($params['Bucket'], $params['Key'], $this->getOptions())) {
            $errors[] = "{$path} already exists on Amazon Scs";
        }
    ...
    public function url_stat($path, $flags)
    {
        ...
        if (!$parts['Key']) {
            // Stat "directories": buckets, or "scs://"
    ...
    /**
     * Remove a bucket from Amazon Scs
     *
    ...
    public function rmdir($path)
    {
        $params = $this->getParams($path);
        if (!$params['Bucket']) {
            return $this->triggerError('You cannot delete scs://. Please specify a bucket.');
        } elseif ($params['Key']) {
     ...
     * Support for opendir().
     *
     * @param string $path    The path to the directory (e.g. "scs://dir[</prefix>]")
    ...
    public function dir_readdir()
    {
        ...
        if (isset($current['Prefix'])) {
            ...
            $key = "scs://{$this->openedBucket}/{$prefix}";
            ...
            $key = "scs://{$this->openedBucket}/{$current['Key']}";
    ...
    public function rename($path_from, $path_to)
    {
        ...
        if (!$partsFrom['Key'] || !$partsTo['Key']) {
            return $this->triggerError('The Amazon Scs stream wrapper only supports copying objects');
        }
    ...
    protected function getOptions()
    {
        ...
        return isset($options['scs']) ? $options['scs'] : array();
    }
    ...
    /**
     * Get the bucket and key from the passed path (e.g. scs://bucket/key)
     *
