{% for it in arrUnitList %}
	<li {% if  it['verification'] == -1 %} class="warn" {% else %} class="warn2" {% endif %}>
	  <div class="col_middle"> 
		...
		<p class=" gray_666">
            ...
            <span class="colwords colw5" id="sp_flush_{{ it['id'] }}"> {% if it['refreshTime'] %}刷新：{{ it['refreshTime'] }} {% else %}尚未刷新{% endif %} </span>
		</p>
	  </div>

	  <div class="col_left"> <input id="CheckBox_{{ it['id'] }} " type="checkbox" class="CheckBoxClass" value="{{ it['id'] }}"  {% if it['verification'] == -1 %} weigui='1'{% else %} weigui='0' {% endif %} /> <label class="radio"  for="CheckBox_{{ it['id'] }}" unitid="{{ it['id'] }}"> <i class="icon_ok"> </i> </label> </div>
